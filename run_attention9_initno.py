...
from initno.pipelines.pipeline_sd_initno_c4 import StableDiffusionInitNOPipeline
...
def process_seed_divide_dataframe(df: pd.DataFrame,
                           top_k: int,
                           root_dir: str,
                           p_idx: int,
                           eps: float = 1e-6,
                           denom_weight: float = 0.5):
    """
    计算 entity / (adjective + other) 平滑比值指标并选出 top_k seeds。

    Parameters
    ----------
    df : pd.DataFrame
        必须包含 entity_mean, adjective_mean, other_mean 列。
    top_k : int
        选取前 top_k 个种子。
    root_dir : str
        输出根目录。
    ...
    eps : float, optional
        分母平滑项，防止除以 0。
    denom_weight : float, optional
        adjective+other 在分母中的权重系数（默认 0.5）。
    """
    # 1️⃣ 归一化 0~1
    for col in ["entity_mean", "adjective_mean", "other_mean"]:
    ...
    # 2️⃣ 反转：adjective / other 越低越好
    df["adjective_mean_norm"] = 1 - df["adjective_mean_norm"]
    df["other_mean_norm"]     = 1 - df["other_mean_norm"]

    # 3️⃣ 平滑比值指标：entity / (eps + w·(adjective + other))
    df["ratio_score"] = (
        df["entity_mean_norm"] /
        (eps + denom_weight * (df["adjective_mean_norm"] + df["other_mean_norm"]))
    )

    # 4️⃣ 按 ratio_score 排序并选前 top_k
    df.sort_values("ratio_score", ascending=False, inplace=True)
    top_seeds = df["seed"].head(top_k).tolist()

    # 5️⃣ 保存
    prompt_dir = os.path.join(root_dir, f"prompt_{p_idx:03d}")
    os.makedirs(prompt_dir, exist_ok=True)
    with open(os.path.join(prompt_dir, f"top{top_k}_seeds.json"), "w") as f:
...
def process_seed_dataframe(df, top_k, root_dir, p_idx):
    for col in ["entity_mean", "adjective_mean", "other_mean"]:
    ...
    # Invert: adjective & other are lower-better
    df["adjective_mean_norm"] = 1 - df["adjective_mean_norm"]
    df["other_mean_norm"] = 1 - df["other_mean_norm"]

    df["composite_score"] = (2 * df["entity_mean_norm"]+ df["adjective_mean_norm"]+ df["other_mean_norm"]) / 4
    ...
    prompt_dir = os.path.join(root_dir, f"prompt_{p_idx:03d}")
    os.makedirs(prompt_dir, exist_ok=True)
    with open(os.path.join(prompt_dir, "top50_seeds.json"), "w") as f:
        json.dump(top_seeds, f, indent=2)
...
def main(base_seed=4):
    ...
    top_k = 50
    root_dir = f"New_comparison/1.5_Token_276prompts_try_all_200steps_only/test{base_seed}"
    ...
    for p_idx, prompt in prompts_dict.items():
        ...
        top_seeds = process_seed_dataframe(df, top_k, root_dir, p_idx)
